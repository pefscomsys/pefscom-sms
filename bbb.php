...
<div style="float:left; width:100px; height:80px;color:#fff;font-size:22px;"><div style="float:left;margin-top:5px;margin-left:5px;">&#9658;</div></div>
 <div style="float:left; width:300px;color:#1188aa; height:30px;font-size:28px;color:#fff;text-transform:uppercase;"><?php echo $roll;?></div></div>
<form action="bbb.php?roll=<?php echo $pic;?>" method="POST" >
...
from semester where id='$amp'  "); 
$row = mysql_fetch_assoc($qry); 
 $saves=$row['total']; 
$rs=mysql_query("Select * from  semester where id='$amp' ");
	  while($row=mysql_fetch_array($rs))
{
if($row[1]==$fees)
{
echo "<option value='$row[0]' selected>$row[4] $row[6]</option>";
}
else
{
echo "<option value='$row[0]'>$row[4] $row[6]</option>";
}
}
?></select></td></tr></table>
 <script>
...
$qry = mysql_query("select description as total
from semester where id='$amp'  "); 
$row = mysql_fetch_assoc($qry); 
echo $save=$row['total']; 
?>
...
<div style="float:left; width:500px;border-bottom:1px solid #000;margin-top:0px; height:30px;margin-left:75px;background:#787878">
			<div style="float:left; width:500px; height:25px;font-size:17px;color:#fff;;">
			<div style="float:left; width:120px;text-align:center;margin-top:4px; height:25px;color:#fff;;">
				Start Date
			</div>
			<div style="float:left; width:20px;text-align:center;margin-top:4px; height:25px;color:#fff;text-transform:uppercase;">
...
$qry = mysql_query("select startdate as total
from semester where id='$amp'  "); 
$row = mysql_fetch_assoc($qry); 
echo $saveu=$row['total']; 
?>
			</div>
...
$qry = mysql_query("select enddate as total
from semester where id='$amp'  "); 
$row = mysql_fetch_assoc($qry); 
echo $saveus=$row['total']; 
?>
			</div>
