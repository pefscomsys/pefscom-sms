...
<form name="look" method="post" action="getsty32.php">
<table style=" border:1px solid silver" cellpadding="2px" cellspacing="0px" align="center" border="0" width="900px">
<tr>
...
$search=$_POST["search"];
$flag=0;
$query="select * from $table  where  db1='$take' and  departmet='$off1' and levels='300'   ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
		 $flag=1;
		 echo "<tr ><td font-color=#000>",$row[2],"</td><td>&nbsp;",$row[3],"</td><td><a href='",$row[1],"'>&nbsp;",$row[5],"
		  </a></td><td><a href='addtotal111cy.php?roll=",$row[0],"'>INDIVIDUAL</a>|
		  <a href='checkst111cy.php?roll=",$row[0],"'>GROUP</a>|<a href='addtotal111cxx.php?roll=",$row[0],"'>Transcript</a>
		   |<a href='addtotal111cxxop.php?roll=",$row[0],"'>Tran Individual</a>
		  </td></tr>";
