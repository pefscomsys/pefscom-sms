...
<?php } ?>
 ...
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='1' and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='2'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='3'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='4'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='5'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='6'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='7'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='8'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php } ?><?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='9'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php
 $notes=$_SESSION['user_name'];
$roll=$_GET["1"];
$query="select * from mainmenu8 where extra='10'  and  borders='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
echo $img=$row['total'];
 ?></h1><br>
	<div style="float:left;color:#000; margin-top:35px; margin-left:-370px;font-size:13px;"><?PHP 
$qry = mysql_query("select  tm as total
from page where id='$ids' and cn='46'  "); 
$row = mysql_fetch_assoc($qry); 
echo $img=$row['total'];
 ?></div>
...
<div style="float:left; WIDTH:15PX; HEIGHT:25PX;">	<i class="fa  fa-user fa-1x" style="float:left; color:#009966; ;">
  ...
  <a href="logout.php" style="color:#009966;">
		...
		<div style="float:left;color:#000; margin-top:15px;  width:190px; HEIGHT:20PX; BACKGROUND:#efefef; margin-left:665px;font-size:13px;">
			Position:-Student Affairs Office
		</DIV>
...
Track Activities
  ...
  width: 40px;
  height: 25px;">
  <i class="fa  fa-print fa-2x" style="float:left; color:#50a3a2; ;">
...
<div id="form" style="width: 1300px; border:0px;">
    <div class="w2ui-page page-0">
        <div style="width: 580px; float: left; margin-right: 0px;">
            <div style="padding: 3px; font-weight: bold; color: #777;"> Print idcards for that department</div>
...
<select name="subject">
<?php
$rs=mysql_query("Select * from subject  where db1>''  and id='$ids'order by  department ASC",$link);
	  while($row=mysql_fetch_array($rs))
{
if($row[1]==$fees)
{
echo "<option value='$row[2]' selected>$row[1]-$row[3]-$row[4]</option>";
}
else
{
echo "<option value='$row[2]'>$row[1]-$row[3]-$row[4]</option>";
}
}
?></select>
                    </div>
        ...
        <button class="w2ui-btn" name="save" size="250px">Process List</button>
    </div>
</div>
