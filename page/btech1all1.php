...
$result=mysql_query($query);
		 ...
		 <?php
		 $t2=$row["departmet"]; $deptt=$row["departmet"];
		 $db1=$row["db1"];
		 $mki=$db1;$level=$row["levels"]; $take=$db1;
		 $t1=$row["levels"];$mat=$row["matricule"];$mats=$row["matricule"];
		 ?>
		 
		 <?php } ?>
		 
		 <?php
$query="select * from $table where levels='$t1' and db1='$db1' and matricule='$mat'  and departmet='$t2'order by fname ASC";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<?php echo $row['departmet'];?>
		 ...
		 text-align:center; margin-top:0px;color:#000; font-weight:bold;">
		  ...
		  <div style="float:left;margin-top:4px;width:800px;text-align:center;"> FIRST SEMESTER - BTECH </div>
		  </div>
