...
if ($_FILES[csv][size] > 0) { 
    ...
    do { 
        if ($data[0]) { 	
/********************************ADDED BY ME*****************/
		 $check="SELECT * from rresit where roll='".addslashes($data[0])."' AND fname= '".addslashes($data[1])."' AND matricule= '".addslashes($data[2])."'AND sex= '".addslashes($data[5])."' AND departmet= '".addslashes($data[3])."'AND levels= '".addslashes($data[4])."' AND db1= '".addslashes($data[6])."' ";
...
echo "<script>alert('Recorded Successfully!'); </script>";
 echo  $statusf="<meta http-equiv='refresh' content='0; url=../edit.php?roll=hgfrtt&amp=11&pic=EDITING / CORRECTION'>";
...
<title>Import a CSV CA'S Marks</title> 
</head> 
<body> 

<div class="upload">
<h1 >Import a CSV CA'S Marks</h1><br />
