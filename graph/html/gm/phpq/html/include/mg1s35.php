...
<?php
$roll=$_GET["1"];
$query="select * from classes where  roll='5'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
$class=$_POST["class"];
$roll=$_GET["1"];
$query="select * from $table where  db1='$take'  and levels>=300 && levels<=300 and  departmet='$off1in5' order by departmet asc";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) { 
