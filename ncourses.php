...
<div id="popup">
<div class="row" >
  <div class="col-md-3" style='width:100%;height:600px;margin-top:15px;'>
<blockquote style='float:left; margin-left:10px;width:100%;height:100px;'>
  <h2 style='color:red;'>Add New Course</h2>
  <small>This include the course description,course code<cite title="Source Title"></cite></small>
</blockquote>

<div style='width:100%;height:300px;'>

<div class="panel panel-default"  style='float:left; width:60%;margin-left:10px; height:360px;'>
  <div class="panel-body">
 <div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="tablesdws.php"  style='height:340px;'></iframe>
  </div>
  </div>
</div>
...
<div   style='float:left; width:400px;margin-left:10px; margin-top:-90px;height:300px;'>
...
<ul class="nav nav-pills" style='margin-top:-25px;margin-left:25px;'><a href='dean.php'><button type="button" class="close" aria-label="Close">
  ...
  <li class="active"><a href="#" style='color:#fff;'><span class="badge">Edit</span></a></li>
  <li><a href="sdepartment.php">Edit Specific Department<span class="badge"></span></a></li>
  ...
  <div class="panel-body">
     <?php
  $db1=$_POST["cdesc"];
  $subject=$_POST["subject"]; $acc=$_POST["acc"];$department=$_POST["department"];
   $year1=$_POST["year1"];
    $year2=$_POST["year2"];
     $extra=$_POST["extra"];
	  $status=$_POST["status"];
...
$qry = mssql_query("SELECT CAST(ids as text) as totals,CAST(deptf as text) as deptf
from [cg2015].[dbo].[classes] where classs='$department' and [level]='$year1' "); 
$row = mssql_fetch_assoc($qry); 

 $idss=$row['totals']; $deptf=$row['deptf'];

$query="insert into [cg2015].[dbo].[subject] 
(db1,subject,department,year1,year2,acc,extra,id,ids,[status])
values
('$db1','$subject','$department','$year1','$year2','$acc','$extra','$idss','$deptf','$status')
...
echo "<script>alert('Subject Recorded!'); </script>";

 echo "<meta http-equiv='refresh' content='0; url=ncourses.php'>";
  ...
  <div class="form-group">
    <label for="exampleInputEmail1">Description of course</label>
    <input type="text" class="form-control" name='cdesc' id="exampleInputEmail1" placeholder="Description of course" required='required'>
    <label for="exampleInputEmail1"  style='width:200px;'>Course Code</label>
    <input type="text" class="form-control" name='subject'  style='width:250px;' id="exampleInputEmail1" placeholder="Course Code" required='required'>
	
    <label for="exampleInputEmail1"style='width:140px;'>Academic Year</label>
    <input type="text" class="form-control" name='acc'style='width:190px;' id="exampleInputEmail1" placeholder="Academic Year" required='required'>
	
    <label for="exampleInputEmail1"style='width:140px;'>Status</label>
    <input type="text" class="form-control" name='status'style='width:190px;' id="exampleInputEmail1" placeholder="C,E,R" required='required'>
	
    <label for="exampleInputEmail1"style='width:90px;'>Start Levels</label> <select class="form-control"name='year1' id="select">
         <?php
...
?> <option value='<?php echo $row["levels"];?>'><?php echo $row["levels"];?></option>
    ...
    <input type="text" class="form-control" name='extra' id="exampleInputEmail1" placeholder="Credit Value" required='required'>
	
  <div class="form-group">
    <label for="exampleInputPassword1" required='required'>Semester</label>
      <div >
        <select class="form-control"name='year2' id="select">
         <?php
 $query="select CAST(sems as text) as ids,CAST(db1 as text) as school from [cg2015].[dbo].[semester] where sems<'3'";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?> <option value='<?php echo $row["ids"];?>'><?php echo $row["school"];?></option>
  ...
  <div class="form-group">
    <label for="exampleInputPassword1" required='required'>Department of Interest</label>
      <div >
        <select class="form-control"name='department' id="select">
         <?php
 $query="select CAST(ids as nvarchar(400)) as ids,CAST(classs as text) as school from [cg2015].[dbo].[classes] order by ids";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?> <option value='<?php echo $row["school"];?>'><?php echo $row["school"];?>  <b style='font-weight:normal;color:#ccc;'> (<?php $nm= $row["ids"];
...
?> )</b></option>
  ...
  <button type="submit" class="btn btn-danger" style='width:180px;'>Save</button>
</form>
  </div>
</div>
</div>
