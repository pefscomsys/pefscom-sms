...
<blockquote style='float:left; margin-left:10px;width:65%;height:100px;'>
  <h2>Users Registered in the System</h2>
...
<div class="panel panel-default"  style='float:left; width:60%;margin-left:10px; height:300px;'>
 ...
 <div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="tablesdus.php?levels=<?php echo $levels;?>"  style='height:280px;'></iframe>
  </div>
  </div>
</div>
...
<ul class="nav nav-pills" style='margin-top:-25px;margin-left:25px;'><a href='omegahome.php'><button type="button" class="close" aria-label="Close">
  ...
  $user_name=$_POST["user_name"]; $password=$_POST["password"];$email=$_POST["email"];
   $level=$_POST["level"];
    $fax=$_POST["location"];
...
$qry = mssql_query("select CAST(full_name as text) as total
from [cg2015].[dbo].[users] where full_name='$fullname'  "); 
$row = mssql_fetch_assoc($qry); 
 ...
 $query="insert into [cg2015].[dbo].[users] 
(full_name,user_name,pwd,[user_level],banned,approved,fax)
values
('$fullname','$user_name','$sha1pass','$level','0','0','$fax')
...
echo "<script>alert('New User Added!'); </script>";
 ...
 echo "<meta http-equiv='refresh' content='0; url=registrar.php?level=$fees'>";
  ...
  <div class="form-group">
    <label for="exampleInputEmail1">Full Name(s)</label>
    <input type="text" class="form-control" name='fullname' id="exampleInputEmail1" placeholder="Full names" required='required'>
    <label for="exampleInputEmail1">Username</label>
    <input type="text" class="form-control" name='user_name' id="exampleInputEmail1" placeholder="User Names" required='required'>
    <label for="exampleInputEmail1">Password</label>
    <input type="text" class="form-control" name='password' id="exampleInputEmail1" placeholder="Password" required='required'>
    <label for="exampleInputEmail1">E-mail</label>
    <input type="text" class="form-control" name='email' id="exampleInputEmail1" placeholder="E-mail" required='required'>
    ...
    <label for="exampleInputPassword1">Location</label>
      <div >
        <select class="form-control"name='location' id="select">
         <?php
 $query="select CAST(location as text) as school,CAST(description as text) as description from [cg2015].[dbo].[menut] where level='1'";
