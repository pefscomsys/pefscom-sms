...
$search=$_POST["search"];
$flag=0;
$query="select * from $table  where  db1='$take'  and c110='$ids' and  departmet='$rem' and levels='400'  ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
		 $flag=1;
		 echo "<tr ><td font-color=#000>",$row[2],"</td><td>&nbsp;",$row[3],"</td><td><a href='",$row[1],"'>&nbsp;",$row[5],"
		  </a></td><td><a href='addtotal111cyp.php?roll=",$row[0],"'>INDIVIDUAL</a>|
		  <a href='checkst111cyp.php?roll=",$row[0],"'>GROUP</a>
		 </td></tr>";
