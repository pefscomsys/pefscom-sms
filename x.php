...
$user_name=$notes;
if(@$_SESSION['userlevel']!=91){header("location: login.php?error=Unauthorized access");}
...
<div id="popup" >
<p style="font-size:20px;">Welcome: <?php echo $notes;?><br>
...
<div style="float:left; width:100px; height:80px;color:#fff;font-size:22px;"><div style="float:left;margin-top:5px;margin-left:5px;">&#9658;</div></div>
 <div style="float:left; width:500px;color:#1188aa; height:30px;font-size:28px;color:#fff;">HOME PAGE  (SECRETARY)</div></div>

 
 
 
			<table width="220px" cellpadding="4" style="margin-left:70px;">
			
						<tr><td></td><td></td></tr>	
				
				
<?php
$query="select * from menu where level='$levels'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
?>
...
<tr  style="border-bottom:1px solid #ccc;"><td style="float:left;text-align:left;">&#9658;</td><td style="float:left;text-align:left;"><a href="<?php echo $row["link"];?>?roll=<?php echo $row["code"];?>&amp=<?php echo $levels;?>&pic=<?php echo $row["faculty"];?>"><?php echo $row["page"];?></a>	
					</td></tr>
