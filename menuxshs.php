...
<title><?php echo $school;?></title>
        <link rel="shortcut icon" href="pef.png"> <script language="javascript" type="text/javascript">
...
function secondPassed() {
	...
	if (seconds == 0) {
		...
		document.getElementById('countdown').innerHTML; window.location.href='scanvotes.php';
...
from barcode where barcode='$txtbarcode' "); 
$row = mysql_fetch_assoc($qry); 
$matricule=$row['total'];
 

$query="select * from courses where   matricule='$matricule' and db1='$take' ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<div style="float:left; width:250px; height:25px; TEXT-ALIGN:RIGHT; fieldset:1px;">
VOTING FOR THE POSITION OF:
</div>
...
from countelec where candidate='$txtbarcode' and extra='$take'  and cname='$xxc' and valid='$values' "); 
$row = mysql_fetch_assoc($qry); 
 $valuess=$row['total'];
$mxc111yts=$valuess; 
 ...
 $statusfbiooputss="";
  ...
  elseif($mxc111ytss>'' ){
   $statusfbiooputss="YOU HAVE VOTED BEFOR FOR!";
  }
...
if(empty($mxc111yts)){
 $statusfbiooputs="insert into countelec set  codes='$css',extra='$take',counts='1', date='$ff', time='$times', cname='$xxc' , department='$department', valid='$values', candidate='$txtbarcode'";
   $statusfbiooputsss="update refrence set  matricule='$xxc',student_name='$css' where roll='1'";
}
  elseif($mxc111yts>'' ){
   $statusfbiooputs="";
   $statusfbiooputsx="CHEAT VOTE BEFOR!";
  }
...
";
$query8cf="$statusfbiooputsss";

mysql_query ($query8c) or die ("");
mysql_query ($query8cf) or die('could not updated:'.mysql_error());
}
echo $mji;
...
<div style="float:left; width:230px;margin-left:470px;border:1px solid #fff;
height:325px; margin-top:-220px;">

<div style="float:left; width:270px; height:25px; TEXT-ALIGN:left;color:#000;color:#fff;">
<div style="float:left; width:250px; height:40px; TEXT-ALIGN:left;color:#fff;font-size:38px;">
<?PHP 	
  echo $css;?>
</div>

<div style="float:left; width:250px; height:40px; TEXT-ALIGN:left;color:#fff;font-size:17px;">
...
from election where codes='$css'  "); 
$row = mysql_fetch_assoc($qry); 
  echo $mxccs=$row['total'];?>
</div>	

<div style="float:left; width:270px; height:25px; TEXT-ALIGN:left;color:#fff;">
AS HIMSU-<?PHP echo $values;?>
...
$row = mysql_fetch_assoc($qry); 
  $mxcc=$row['total'];
				...
				$qry = mysql_query("select id as total
from employees where empname='$mxcc'"); 
$row = mysql_fetch_assoc($qry); 
 $savep44=$row['total']; 
				 $mxx=$rec['empname'];
				 ?>
					...
					<img src="load_image.php?pic_id=<?php echo $savep44;?>" width="220px" height="170px"style="border:2px solid #ccc; box-shadow:2px 2px 2px 2px #000;opacity:0.7;">
...
$row = mysql_fetch_assoc($qry); 
$valuess=$row['total'];?>
<div style="float:left; width:230px; TEXT-TRANSFORM:UPPERCASE; height:30px; TEXT-ALIGN:left;color:#fff;background:<?php echo $valuess;?>;">

<?php echo $valuess;?>-PARTY
</div>	
				</div>	
