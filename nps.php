...
<div id="popup">
<div class="row" >
  <div class="col-md-3" style='width:110%;height:750px;margin-top:15px;'>
<blockquote style='float:left; margin-left:10px;width:75%;height:100px;'>
  <h2 style='color:red;'>View Courses base on selection</h2>
  <small>This include selecting the school and department<cite title="Source Title"></cite></small>
</blockquote>

<div style='width:110%;height:300px;'>

<div class="panel panel-default"  style='float:left; width:800px;margin-left:10px; height:560px;'>
  <div class="panel-body">
 <div class="embed-responsive embed-responsive-16by9">
    <iframe class="embed-responsive-item" src="tablesdwss.php?school=<?php echo $_GET["school"];?>&dept=<?php echo $_GET["dept"];?>&names=<?php echo $_GET["names"];?>"></iframe>
  </div>
  </div>
</div>
...
<ul class="nav nav-pills" style='margin-top:-25px;margin-left:25px;'><a href='dean.php'><button type="button" class="close" aria-label="Close">
  ...
  <li class="active"><a href="#" style='color:#fff;'><span class="badge">Select</span></a></li>
  <li><a href="#">Selecting School<span class="badge"></span></a></li>
  
</ul>

  <div class="panel panel-primary"style='margin-top:45px;'>
  <div class="panel-heading">
    <h3 class="panel-title"  style='color:white;font-size:21px;'>Department's of the Institution</h3>
  </div>
 

<?php
$school=$_GET['school'];
 $query="select CAST(school as text) as school,CAST(ids as text) as code from [cg2015].[dbo].[school] where ids='$school' ";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?>
   
  
<div class="panel-body" style='text-align:left;'><a href="np.php?school=<?php echo $row["code"];?>">
   <?php echo $row["school"];?></a>
  </div>
...
<?php
$school=$_GET["school"];
 $query="select CAST(classs as text) as school,CAST(ids as text) as code from [cg2015].[dbo].[classes]
 where ids='$school'";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?>
  <div class="panel-body">
  <a href="nps.php?school=<?php echo $school;?>&dept=<?php echo $row["code"];?>&names=<?php echo $row["school"];?> " > <?php echo $row["school"];?> <b style='font-weight:normal;color:#ccc;'>(<?php $nm= $row["code"];
