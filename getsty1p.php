...
<form name="look" method="post" action="getsty1p.php">
<table style=" border:1px solid silver" cellpadding="2px" cellspacing="0px" align="center" border="0" width="1050px">
<tr>
...
$search=$_POST["search"];
$flag=0;
$query="select * from $table  where  db1='$take'  and c110='$ids' and  departmet='$rem' and levels='100'  ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
		 $flag=1;
		?><tr ><td font-color=#000><?php echo $row[2];?></td><td><?php echo $row[3];?></td><td><a href='<?php echo $row[1];?>'><?php echo $row[5];?>
		  </a></td><td><a href='addtotal111cy.php?roll=<?php echo $row[0];?>'>INDIVIDUAL</a>|
		  <a href='checkst111cy.php?roll=<?php echo $row[0];?>'>GROUP</a>|	  <a href='
		 <?php
		 $ls=$row["levels"];
		 $myhuuu=$ids; 
if(empty($myhuuu)){
 $myhuuu='';}elseif($myhuuu>='15' && $myhuuu<='15' 
&& $ls>='100' && $ls<='100'){
$tmms="printu1xm.php";
$tmms2="printu23x.php";
...
elseif($myhuuu>='15' && $myhuuu<='15' 
&& $ls>='400' && $ls<='400'){
$tmms="chaper1bss1.php";

}
echo	$tmms;	 
?>?roll=<?php echo $row[0];?>'>Transcript</a>
		 </td></tr>
<?php
		 }
		 if($flag==0)
