...
?>
	...
	?>
                 <a 
style='text-decoration:none; font-size:19px; background:#efefef; width:400px;'  onclick="window.open('pcourses.php?dept=<?php echo $dept;?>&school=<?php echo $school;?>&names=<?php echo $names;?>', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');">
Print List

</A>
              <table id="mytable" class="table table-bordred table-striped" style='with:600px;'>
	...
	$query="select CAST(db1 as text) as from1,CAST(subject as text) as from2,CAST(department as text) as gpa,
	 ...
	 CAST(extra as text) as extra from [cg2015].[dbo].[subject] where id='$school' and department='$names' ";
