...
<?php
$roll=$_GET["1"];
$query="select * from matsub where mgg='$off1in' and levels='400'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<form name="look" method="post" action="getst43.php">
...
$search=$_POST["search"];
$flag=0;
$query="select * from $table where  db1='$take' and  departmet='INSURANCE' and levels='400'  ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
		 $flag=1;
		 echo "<tr ><td font-color=#000>",$row[2],"</td><td>&nbsp;",$row[3],"</td><td><a href='",$row[1],"'>&nbsp;",$row[5],"
		  </a></td><td><a href='addtotal111cf4a.php?roll=",$row[0],"'>INDIVIDUAL</a>|<a href='checkst111cf4a.php?roll=",$row[0],"'>GROUP</a>
		  </td></tr>";
