...
<?php
$mats=$_GET['mats'];
$date=$_GET["date"];$folk=$_GET["folk"];$fname=$_GET["fname"];$db1=$_GET["db1"]; $db1s=$_GET["db1"];
...
verdana;  border-radius:25px 25px 25px 25px; border:1px solid #000;">
<h4 style='margin-top:-3px; padding-top: 15px;'><?php echo $school;?></h4><div style="float:left;margin-top:-20px;">
...
<?php
 ...
 CAST(name as text) as name, CAST(amount as float) as amount, CAST(amount2 as float) as amount2, CAST(folk as float) as folk

, CAST(balance as float) as balance
, CAST(date as text) as date, CAST(receiptno as float) as receiptno

 from [cg2015].[dbo].[balancesheet] where  db1='$db1s'  and fname='$mats' and date='$date' and folk='$folk' ";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
...
<div style=" float:left; width:700px; margin-top:1px;TEXT-ALIGN:CENTER;  height:34px; 
font-size:28px; ">
<div style=" float:left; width:500px; margin-top:1px;TEXT-ALIGN:CENTER;  height:34px; 
font-size:28px; ">
CASH RECEIPT 

</div>

<div style=" float:left; width:200px; margin-top:17px;TEXT-ALIGN:CENTER;  height:34px; border: 1px; solid black;
font-size:18px; background-color: #aaa;">
N<SUP>o</SUP> 00<?PHP ECHO $row['receiptno'];?>
...
<?php 
//process the date. get day, month and year frm  the date passed in
 $dates;
$day = substr($date, 0, 2);
...
</div><div  style='float:left; width:160px; height:25px; font-weight: bold;'>Amount</div>
...
REGISTRATION FEE</div><div  style='float:left; width:100px; height:<?php echo $statusht2;?> ;'><?php echo number_format($mxc112);?></div>
