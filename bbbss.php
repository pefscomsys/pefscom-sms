...
<div style="float:left; width:100px; height:80px;color:#fff;font-size:22px;"><div style="float:left;margin-top:5px;margin-left:5px;">&#9658;</div></div>
 <div style="float:left; width:400px;color:#1188aa; height:30px;font-size:28px;color:#fff;text-transform:uppercase;"><?php echo $deptt;
 
 
$qry = mysql_query("select comment as total
from  department where comment='$deptt' "); 
...
$row = mysql_fetch_assoc($qry); 
 $saves=$row['total']; 
$rs=mysql_query("Select * from  department where comment='$deptt' ");
...
$rs=mysql_query("Select * from  levels where cat='$level' ");
	  while($row=mysql_fetch_array($rs))
{
if($row[1]==$fees)
{
...
$query="select * from subject where department='$deptt'  and year2='1' and year1='$level' and db1>''  order by subject ASC";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
?>
...
from credits where subject='$ss'  "); 
$row = mysql_fetch_assoc($qry); 
echo $credit=$row['total']; 
		
		
		
$query3r="update subject set credit='$credit',status='$status'
 where subject='$ss' ";
...
<div style="float:left; width:620px;border-bottom:1px solid #000;margin-top:0px; height:30px;margin-left:15px;background:rgb(120,80,100);">
			<div style="float:left; width:6100px; height:25px;font-size:17px;color:#fff;text-transform:uppercase;">
			...
			<div style="float:left; width:110px;text-align:center;margin-top:4px; height:25px;color:#fff;text-transform:uppercase;">
			...
			<div style="float:left; width:160px;text-align:left;margin-top:4px; height:25px;color:#fff;">
		Total Credit : <?php 
$qry = mysql_query("select SUM(credit) as total
from subject where department='$deptt'  and year2='1' and year1='$level'  "); 
$row = mysql_fetch_assoc($qry); 
echo $credits=$row['total']; ?>
			</div>
...
$query="select * from subject where department='$deptt'  and year2='2' and year1='$level'  and db1>''order by subject ASC";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
?><a href="ccoum.php?id=<?php echo $row["roll"];?>&dept=<?php echo $row["department"];?>&levels<?php echo $row["year1"];?>&ccode=<?php echo $row["db1"];?>">
...
<div style="float:left; width:620px;border-bottom:1px solid #000;margin-top:0px; height:30px;margin-left:15px;background:rgb(120,80,100);">
		...
		Total Credit : <?php 
$qry = mysql_query("select SUM(credit) as total
from subject where department='$deptt'  and year2='2' and year1='$level'  "); 
$row = mysql_fetch_assoc($qry); 
echo $credits=$row['total']; ?>
			</div>
