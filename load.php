...
$sex=$_POST["sex"];
		...
		$insert="INSERT INTO rresits (fname,matricule,departmet,levels,sex,db1,c101) VALUES 
                ( 
                   '".addslashes($data[0])."', 
                    '".addslashes($data[1])."',
					'".addslashes($data[2])."', 
...
echo "<script>alert('Recorded Successfully!'); </script>";
 echo  $statusf="<meta http-equiv='refresh' content='0; url=sendah.php?levels=$levels&sex=$sex&dept=$dept&sub=$sub'>";
	 die(mysql_error()); 
...
<title>Import a CSV Exam Marks</title> 
</head> 
<body> 

<div class="upload">
<h1 >Import a CSV Exam Marks</h1><br />
...
<input name="csv" type="file" id="csv" /> 
 <input type='hidden' name='dept' size='8' value='<?php
 
 echo $dept;?>'  maxlength='2'>
 
  <input type='hidden' name='sex' size='8' value='<?php
 
 echo $sex;?>'  maxlength='2'>
 
  <input type='hidden' name='levels' size='8' value='<?php
 
 echo $levels;?>'  maxlength='2'>
 
  <input type='hidden' name='sub' size='8' value='<?php
 
 echo $sub;?>'  maxlength='2'>
