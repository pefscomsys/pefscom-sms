...
<ul class="nav nav-pills" style='margin-top:-25px;margin-left:25px;'><a href='dean.php'><button type="button" class="close" aria-label="Close">
  ...
  <div class="panel-heading">
    <h3 class="panel-title"  style='color:white;font-size:21px;'>Select Department under this Institution</h3>
  </div>
 

<?php
$school=$_GET["school"];
 $query="select CAST(facultydesc as text) as school,facultyid as code from [cg2015].[dbo].[faculty] where facultyid='$school'";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?>
...
<?php
$school=$_GET["school"];
 $query="select CAST(classs as text) as school,CAST(ids as text) as code from [cg2015].[dbo].[classes] where ids='$school'";
$result=mssql_query($query);
		 while ($row = mssql_fetch_array($result)) {
?>
  <div class="panel-body">
  <a href="nps.php?school=<?php echo $school;?>&dept=<?php echo $row["school"];?> " > <?php echo $row["school"];?>   
  
  
   <b style='font-weight:normal;color:#ccc;'> (<?php $nm= $row["code"];
