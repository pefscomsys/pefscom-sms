...
$search=$_POST["search"];
$flag=0;
$query="select * from balancesheet where   ass='$ids'  and status like '%$search%'  and fname>''  order by date DESC limit 30 ";
$result=mysql_query($query);
$i=0;
 while ($row = mysql_fetch_array($result)) {
if($i%2==0)
...
<?php echo $mats=$row["fname"]; $status=$row["status"]; $cats=$row["name"];$db1=$row["db1"];$roll=$row["recid"];?> ,level <?PHP 
$qry = mysql_query("select levels as total
from courses where matricule='$mats'  and db1='$db1'  "); 
$row = mysql_fetch_assoc($qry); 
 echo $sav4=$row['total'];  ?></td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $cats; ?> </td>
...
<td >
<a href="deletest.php?roll=<?php echo $roll; ?>&uio=<?php echo $mats; ?>&db1=<?php echo $db1; ?>">Delete Receipt</a></td>
</tr></a>
<?php
$i++;
}
