...
mysql_query("UPDATE courses set matricule='" . $_POST["userName"][$i] . "', departmet='" . $_POST["new"][$i] . "' 
 WHERE roll='" . $_POST["userId"][$i] . "'");
 
 
 mysql_query("UPDATE admission set matricule='" . $_POST["userName"][$i] . "'
 WHERE recid='" . $_POST["u1"][$i] . "'");
 
 
 
 
 mysql_query("UPDATE historic set matricule='" . $_POST["userName"][$i] . "', amountpaid='" . $_POST["new"][$i] . "' 
 WHERE roll='" . $_POST["rolf"][$i] . "'");
 
  
 mysql_query("UPDATE balancesheet set fname='" . $_POST["userName"][$i] . "'
 WHERE recid='" . $_POST["rolff"][$i] . "'");
 
 
   
 mysql_query("UPDATE barcode set matricule='" . $_POST["userName"][$i] . "'
 WHERE recid='" . $_POST["rolfff"][$i] . "'");
...
echo "<script>alert('Change Updated Successful!'); </script>";
 ...
 echo $statusftydate="<meta http-equiv='refresh' content='0; url=changedepttp.php'>";
 }
...
<td><input type="text" name="new[]" class="txtField" value="<?php echo $row[$i]['departmet']; ?>"></td>
...
$row = mysql_fetch_assoc($qry);
 ...
 ?>" size='28'>
  <input type="text" name="u1[]" class="txtField" value="<?php 
   
$qry = mysql_query("select recid as total
from admission where matricule='$mats'   "); 
$row = mysql_fetch_assoc($qry);
 echo $msk=$row['total']; 

 ?>" size='28'>
 
  <input type="text" name="rolff[]" class="txtField" value="<?php 
...
$row = mysql_fetch_assoc($qry);
 ...
 ?>" size='28'>
   ...
   <input type="text" name="rolfff[]" class="txtField" value="<?php 

$qry = mysql_query("select recid as total
from barcode where matricule='$mats'  and db1='$take' "); 
$row = mysql_fetch_assoc($qry);
 echo $msk=$row['total']; 
