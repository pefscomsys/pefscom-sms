...
<form name="form1" action="200m.php?roll=<?php echo $fish;?>" method="post" enctype="multipart/form-data" >
...
<?php
$rs=mssql_query("Select CAST(levels as text) as levels from levels");
	  while($row=mssql_fetch_array($rs))
{
if($row[1]==$fees)
{
echo "<option value='$row[levels]' selected>$row[levels]</option>";
}
else
{
...
<?php
$rs=mssql_query("Select CAST(db1 as text) as db1 from courses group by db1",$link);
	  while($row=mssql_fetch_array($rs))
{
if($row[1]==$fees)
{
