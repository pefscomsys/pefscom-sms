...
<td class="ExcelTableFormationCol" style="font-weight:bold;">S/N</td>
 <td class="ExcelTableFormationCol"style="font-weight:bold;">Student Names</td>
  ...
  <td class="ExcelTableFormationCol"style="font-weight:bold;">Expected Amount</td>
  
  <td class="ExcelTableFormationCol"style="font-weight:bold;">Schorlarship</td>
...
$query = mssql_query("select  CAST(student_name as nvarchar(400)) as fname,CAST(balance as text) as balance,
CAST(amount_paid as text) as amount_paid,CAST(expected_amount as text) as expected_amount, CAST(scholarship as text) as schorlarship, 
CAST(matricule as text) as matricule, CAST(classs as text) as classs from historic where classs='$department' and levels='$levels' and oldb='$db1'
...
<td style='width:60px;text-align:center;'><?php echo number_format($row["schorlarship"]);?></td>
...
<td style='width:100px;text-align:center;'><b> Total Paid</b></td>
...
<td style='width:60px;text-align:center;'><?php 
	 ...
	 $qryg= "select  SUM(CAST(scholarship as float)) as total
 from historic where classs='$department' and levels='$levels' and oldb='$db1' "; 
$tyyi=mssql_query($qryg);
$row = mssql_fetch_assoc($tyyi); 
echo  $xss=number_format($row['total']);
