...
<?php
$roll=$_GET["1"];
$query="select * from matsub where mgg='$off1in'  and ids='$ids'";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
...
<form name="look" method="post" action="getst1pv.php?roll=<?php echo $roll;?>&term=<?php echo $term;?>">
<table style=" border:1px solid silver" cellpadding="2px" cellspacing="0px" align="center" border="0" width="950px">
<tr>
...
<?php
 $search=$_POST["search"];
$flag=0;
$query="select * from $table  where  db1='$take' and  departmet='$rem' and levels='100' and fname like '%search%'  ";
$result=mysql_query($query);
		 while ($row = mysql_fetch_array($result)) {
		 $flag=1;
		 echo "<tr ><td font-color=#000>",$row[2],"</td><td>&nbsp;",$row[3],"</td><td><a href='",$row[1],"'>&nbsp;",$row[5],"
		  </a></td><td><a href='addtotal111c.php?roll=",$row[0],"&id=",$row['c110'],"&term=$term'>INDIVIDUAL</a>|<a href='checkst111c.php?roll=",$row[0],"&id=",$row['c110'],"&term=$term'>GROUP</a>
		  </td></tr>";
